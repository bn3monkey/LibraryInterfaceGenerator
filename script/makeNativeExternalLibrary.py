...
def makeNativeExternalLibrary() :
    current_file_path = os.path.dirname(os.path.abspath(__file__))
    project_path = os.path.join(current_file_path, "..")
    src_path = os.path.join(project_path, "src")
    implementation_path = os.path.join(src_path, "implementation")
    native_external_library_path = os.path.join(implementation_path, "NativeExternalLibraryDirectory")
    ...
    for entry in entries :
        if entry.is_dir() :
            ...
            with open(library_h_path, 'w') as macro_file :
                ...
                macro_file.write("static constexpr char* {0}_header = \n".format(library_name))

                library_hpp_path = os.path.join(library_path, library_name + ".hxx")
                with open(library_hpp_path, 'r') as f :
                    while True :
                        line = f.readline()
                        if not line : break
                ...
                macro_file.write("static constexpr char* {0}_src = \n".format(library_name))
